...
def sendemail(to, content):
    ...
    server.ehlo()
    server.starttls()
    server.login('sharmaashucall@gmail.com', '7788998855') # use snmtp settings for email
    server.sendmail('sharmaashucall@gmail.com', to, content)
    server.close()
...
def play():
    ...
    while True:
        ...
        if 'exit' in query:
            ...
            var.set("Myself Mozo Sir")
            window.update()
            speak('myself Mozo sir')
            ...
            var.set('Hello Everyone! My self mozo')
            window.update()
            speak('Hello Everyone! My self mozo')
